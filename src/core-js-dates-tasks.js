...
function getNextFriday(date) {
  ...
  const nextFriday = new Date(inputDate);
  nextFriday.setUTCDate(inputDate.getUTCDate() + daysUntilFriday);
  return nextFriday;
}
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${formattedHour}:${pad(minute)}:${pad(second)} ${ampm}`;
}
